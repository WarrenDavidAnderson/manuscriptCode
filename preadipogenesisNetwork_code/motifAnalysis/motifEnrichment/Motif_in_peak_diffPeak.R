...
dir = paste0("/media/wa3j/Seagate2/Documents/PRO/",
             "adipogenesis/July2018/atac_time_deg/motifEnrich/deg")
setwd(dir)
...
# see atac_time_deg_meme.R (res.pairs list)
...
# see enrichSummary_diffPeak.sh for analysis 

############################################################
# perform enrichment analyses for each factor
# see enrichSummary_diffPeak.sh for the implementation of this analysis on Rivanna
...
# cd /scratch/wa3j/enrich_diffPeak/process
# module load gcc/7.1.0 openmpi/3.1.4 R/3.5.3
# fimo.bigWig.path = paste0("/scratch/wa3j/f750000")

# load fac.chisq
load("enrichdat_600bp_step20.RData")
load("enrichdatProcessed750k.RData")
...
# f = (1/(sig*(2*pi)^(1/2)))*exp(-0.5*((x-mu)/sig)^2)
# plot(x,f,type="l")
# plot(x,f-mean(f),type="l")
# trapz(x,f)
# trapz(x,(f-mean(f)))
...
# check fdrs
max(enrich.inc$pval) # 4.714301e-22
max(enrich.dec$pval) # 2.806882e-07
max(enrich.un$pval)  # 9.656527e-14
...
write.table(enrich.summary,"enrichSummary750k_deg.txt",col.names=T,row.names=F,quote=F,sep="\t")
...
plot.enrich(comp.mode="inc",fname="enrichINCfiltered750k.pdf",mfrow=c(3,3),
            ...
            sig.un=sig.un, fc.un=fc.un, type="deg",
...
# nohup Rscript --vanilla script2.R &
# Submitted batch job 6640275
...
pdf("TFs_filtered750k.pdf", onefile = TRUE, height=9, width=4.5)
...
write.table(TFspreadsheet,"TFclusters750k_deg.txt",col.names=T,row.names=F,sep="\t")
write.table(motif.community5,"TFclusterAnnotation750k_deg.txt",col.names=T,row.names=F,sep="\t")
...
plt.comms = c(77,79,8,200,86,92,62,385,354,137,399,93)
